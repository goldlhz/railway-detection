...
VOID ServiceMainProc()
{
	...
	if(dwSize > 0)
	{
		strcpy(pServiceName,"RailwayService");
...
{  
	...
	if (schSCManager==0) 
	{
		::MessageBox(NULL, _T("注册系统服务过程中，打开服务控件管理器出错，注册失败！"), _T("出错"), MB_OK);
		return;
	}
	else
	{
		...
		if (schService==0) 
		{
			::MessageBox(NULL, _T("注册系统服务过程中，写入系统服务信息出错，注册失败！"), _T("出错"), MB_OK);
			return;
		}
		else
		{
			::MessageBox(NULL, _T("注册系统服务RailwayService成功！"), _T("成功"), MB_OK);
...
VOID UnInstall(char* pName)
{
	...
	else
	{
		...
		else
		{
			...
			{
				::MessageBox(NULL, _T("删除系统RailwayService服务失败！"), _T("出错"), MB_OK);
			}
			else 
			{
				::MessageBox(NULL, _T("删除系统RailwayService服务成功！"), _T("成功"), MB_OK);
			}
			CloseServiceHandle(schService); 
		}
		CloseServiceHandle(schSCManager);	
