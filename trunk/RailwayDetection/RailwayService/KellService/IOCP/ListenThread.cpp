...
bool CListenThread::InitListener(CGobalConfig * pGobalConfig)
{
	ASSERT(pGobalConfig);
...
void CListenThread::UnInitListener()
{
	ReleaseAllSource();
	SafeTerminate();
}
...
bool CListenThread::CreateIOCPHandle(CGobalConfig * pGobalConfig)
{
	if(!m_hCompletionPort && pGobalConfig)
	{
		int nErrorCode = 0;
		int nThreadCount = CCommonFunction::GetKernalCount() * 2 + 2;
...
bool CListenThread::PostAcceptExMSG()
{
	...
	if((INVALID_SOCKET != sClientSocket) && (NULL != pOverKeyPire))
	{
		...
		bTempTest = m_sListenSocket.AcceptEx(
			pOverKeyPire->pireCompletionKey.keyOptSocket, 
			sClientSocket,
