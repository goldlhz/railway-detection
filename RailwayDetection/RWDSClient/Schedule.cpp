...
BOOL CSchedule::OnInitDialog()
{
	...
	m_ListCtrl.InsertColumn(0, _T("编号"), LVCFMT_LEFT, clientRect.Width()/7);
	m_ListCtrl.InsertColumn(1, _T("排班名"), LVCFMT_LEFT, clientRect.Width()/7);
	m_ListCtrl.InsertColumn(2, _T("线路名"), LVCFMT_LEFT, clientRect.Width()/7);
	...
	m_ListCtrl.InsertColumn(5, _T("开始时间"), LVCFMT_LEFT, clientRect.Width()/7);
	m_ListCtrl.InsertColumn(6, _T("结束时间"), LVCFMT_LEFT, clientRect.Width()/7);
	...
	for (int i=0; i<count; i++)
	{
		...
		m_ListCtrl.SetItemText(i, 2, lineName);
		m_ListCtrl.SetItemText(i, 3, dev);
		m_ListCtrl.SetItemText(i, 4, wk);
		m_ListCtrl.SetItemText(i, 5, beTime);
		m_ListCtrl.SetItemText(i, 6, endTime);
	}
