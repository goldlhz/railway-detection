...
SOCKET CSocketPool::CreatePoolSocket()
{
	...
	if(INVALID_SOCKET != scSocket)
	{
		...
		nErrorCode = setsockopt(scSocket, IPPROTO_TCP, TCP_NODELAY, (const char*)&bNODELAY, sizeof(BOOL));
		if(SOCKET_ERROR  == nErrorCode)
		{
			nErrorCode = WSAGetLastError();
			WriteLogInfo(LOG_INFO, _T("CSocketPool::CreatePoolSocket(), 设置客户套接字Disables the Nagle algorithm for send coalescing,错误代码:%d"), nErrorCode);
